...
for xml in xml/*; do
  docid=${xml#*/}
  docid=${docid%.xml}
  cat ud/$docid-*.txt > ud/$docid.txt
  printf "\rSplitting $xml to sentences... ($i/$total)"
  i=$((i+1))
  python -m scripts.standard_to_sentences $xml --sentences ud/$docid.txt -o sentences_by_ud -b --suffix-format='-%04d' --suffix-start=1 2>/dev/null || exit 1
done
echo
